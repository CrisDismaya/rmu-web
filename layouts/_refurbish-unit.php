               ...
               <div class="row">
						<div class="col-12">
							<div class="page-title-box d-sm-flex align-items-center justify-content-between">
								<h4 class="mb-sm-0">Request Refurbish of Units</h4>
							</div>
						</div>
					</div>
					...
					<div class="card pa-2">
						...
						<div class="row" id="details">
							...
							<div class="col-12 note">
								...
								<span>Note: This request is subject for approval. !</span></center>
							</div>
	...
	<script>
		...
		$(document).ready(function(){
			...
			$('#save-details').click(function(event){
				...
				var url = (id == 0 ? `${baseUrl}/requestRefurbish` : `${baseUrl}/updateRefurbish/${ id }`)
				...
				$.ajax({
					...
					success: function (data) { 
						...
						if(!data.success){
							...
							hideLoader()
							let msg = id == 0 ? 'New Refurbish Request Succesfully submit!' : 'Refurbish Request Succesfully updated!'
							toast(msg, 'success');
				...
				columns: [
					...
					{ data: null, defaultContent: '',
						...
						html = 'No action available';
							if(oData.status == 'WAITING FOR APPROVAL' && tableData.role == 'Maker' && oData.approved_price == null){
								html = `
									No Action Available
								`;
							}
							...
							if(oData.status == 'DISAPPROVED' && tableData.role == 'Maker' && oData.approved_price != null){
								html = `
									...
									onclick="edit(${ oData.repo_id },'${ oData.refurbish_id }', '${ oData.branch }', '${ oData.branchname }', 
										'${ oData.brandname }', '${ oData.repo_id }', '${ oData.model_name }',
										 '${ oData.model_chassis }', '${ oData.model_engine }',
										  '${ tableData.role }','update','${ oData.color }','${ oData.qoute }')"> 
		...
		function viewForApproval(repo_id,refurbish_id,branchid,branchname,brand,repo_id,modelname,chassis,engine,role,categ,color){
			...
			let qoute = qoute_data.filter(d => { return d.qoute_id == refurbish_id})[0]
