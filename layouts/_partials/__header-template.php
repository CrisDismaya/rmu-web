   <meta charset="utf-8" />
	...
	<script>
		...
		function getAllForApproval(){
			$.ajax({
				url: `${baseUrl}/sidebarNotif`, 
				type: 'GET', 
				headers:{
					'Authorization':`Bearer ${ auth.token }`,
				},
				success: function (data) {
					if(data.for_approval > 0){
						$('#for_approval').html(data.for_approval)
						$('#for_approval').show()
					}else{
						$('#for_approval').hide()
					}
		...
		function getNotif(){
			$('#for_stock_transfer').addClass('d-none')
			$('#for_stock_transfer_received').addClass('d-none')

			const data = $.ajax({
				url: `${baseUrl}/getSidebarNotif`,
				method: 'GET',
				dataType: 'json',
				headers:{
					'Authorization':`Bearer ${ auth.token }`,
			...
			data.done(function(res) {
				// console.log(res)
				if(res.stock_transfer > 0){
					$('#stock_transfer').removeClass('d-none').text(res.stock_transfer)
				}

				if(res.receive_stock_transfer > 0){
					$('#receive_stock_transfer').removeClass('d-none').text(res.receive_stock_transfer)
				}
			});
		}
